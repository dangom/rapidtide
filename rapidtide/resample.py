...
def arbresample(
    ...
    if decimate:
        ...
        orig_x = sp.linspace(
            0.0, 1.0 / init_freq * len(inputdata), num=len(inputdata), endpoint=False
        )
        if debug:
            print(
                "arbresample:",
        ...
        return dotwostepresample(
            orig_x, inputdata, intermed_freq, final_freq, method=method, debug=debug
        )
...
def dotwostepresample(
    orig_x, orig_y, intermed_freq, final_freq, method="univariate", debug=False
):
    """
