...
def refineregressor(
    ...
    ampmask = np.where(
        lagstrengths >= optiondict["ampthresh"], np.int16(1), np.int16(0)
    )
    if optiondict["lagmaskside"] == "upper":
        delaymask = np.where(
            lagtimes > optiondict["lagminthresh"], np.int16(1), np.int16(0)
    ...
    if optiondict["dodispersioncalc"]:
        ...
        for lagnum in range(0, np.shape(laglist)[0]):
            ...
            if np.shape(inlagrange)[0] > 0:
                ...
                freqs, dispersioncalcspecmag[lagnum, :], dispersioncalcspecphase[
                    lagnum, :
