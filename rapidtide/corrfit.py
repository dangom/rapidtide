...
def onecorrfitx(
    ...
    if not optiondict["fixdelay"]:
        ...
        maxindex = np.int16(
            bisect.bisect_left(corrscale, optiondict["fixeddelayvalue"])
        )
...
def _procOneVoxelFitcorrx(
    ...
    maxindex, maxlag, maxval, maxsigma, maskval, peakstart, peakend, failreason = onecorrfitx(
        corrtc,
        corrscale,
        optiondict,
        zerooutbadfit=zerooutbadfit,
    ...
    if (maskval == 0) and optiondict["zerooutbadfit"]:
        ...
        if (not optiondict["fixdelay"]) and (maxsigma != 0.0):
            thegaussout = rt_floatset(
                tide_fit.gauss_eval(corrscale, [maxval, maxlag, maxsigma])
            )
        else:
...
def fitcorrx(
    ...
    reportstep = 1000
    volumetotal, ampfails, lagfails, windowfails, widthfails, edgefails, fitfails = (
        0,
        0,
        0,
        0,
    ...
    if optiondict["nprocs"] > 1:
        ...
        for vox in range(0, inputshape[0]):
            ...
            if themask is None:
                ...
                thislag = None
            if dothisone:
                dummy, volumetotalinc, lagtc[vox, :], lagtimes[vox], lagstrengths[
                    vox
                ], lagsigma[vox], gaussout[vox, :], windowout[vox, :], R2[vox], lagmask[
                    vox
                ], failreason = _procOneVoxelFitcorrx(
                    vox,
                    corrout[vox, :],
                    corrscale,
                    genlagtc,
    ...
    if optiondict["nprocs"] > 1:
        ...
        for vox in range(0, inputshape[0]):
            ...
            if dothisone:
                dummy, volumetotalinc, lagtc[vox, :], lagtimes[vox], lagstrengths[
                    vox
                ], lagsigma[vox], gaussout[vox, :], R2[vox], lagmask[
                    vox
                ], failreason = _procOneVoxelFitcorr(
                    vox,
                    corrout[vox, :],
                    corrscale,
                    genlagtc,
